...
const makePanel = (text, info) => `
<div class="hint-info-title" id="hint-panel">
  <span class="title-text">
    ${text}
  </span>
  <a class="title-href" target="_blank" href="/gov/api/insight/anomalyModel/guide/comds.html#${text}">了解更多信息</a>
</div>
<div class="hint-info-content">${info}</div>
`
...
function Widget(completion, data) {
  ...
  hints.className = "CodeMirror-hints " + theme + ` ${completion.options.showHintInfo ? 'show-hint-info' : ''}`;
  this.selectedHint = data.selectedHint || 0;

  this.selectedHintInfo = {
    text: '',
    info: ''
  }
  ...
  if (completion.options.showHintInfo) {
    const hintInfo = ownerDocument.createElement('div')
    hintInfo.style.left = (left - offsetLeft) + 'px'
    hintInfo.style.top = (top - offsetTop + box.height) + 'px'
    if (Array.isArray(completions) && completions.length > 0) {
      const { text, info } = completions[0]
      this.selectedHintInfo = {
        info,
        text
      }
      hintInfo.innerHTML = makePanel(text, info)
    }

    hintInfo.className = 'CodeMirror-hint-info'
    this.hintInfo = hintInfo

    this.clickHandler = e => {
      e.fromHintClick = true
    }

    hintInfo.addEventListener('click', this.clickHandler)
    ...
    this.mouseOverHandler = e => {
      ...
      if (nodeName === 'LI') {
        ...
        const { data } = this
        if (data && Array.isArray(data.list) && data.list[index]) {
          const { text, info } = data.list[index]

          this.hintInfo.innerHTML = makePanel(text, info)
        }
      }
    }
    ...
    this.mouseOutHandler = e => {
      ...
      const nodeName = target.nodeName.toUpperCase()
      if (nodeName === 'UL') {
        const { text, info } = this.selectedHintInfo
        this.hintInfo.innerHTML = makePanel(text, info)
      }
    }

    hints.addEventListener('mouseover', this.mouseOverHandler)

    hints.addEventListener('mouseleave', this.mouseOutHandler)
  }
...
Widget.prototype = {
  close: function () {
    ...
    if (this.hintInfo) {
      this.hintInfo.removeEventListener('click', this.clickHandler)
      this.hintInfo.parentNode.removeChild(this.hintInfo)

      this.hints.removeEventListener('mouseover', this.mouseOverHandler)
      this.hints.removeEventListener('mouseleave', this.mouseOutHandler)
    }
  ...
  changeActive: function (i, avoidWrap) {
    ...
    if (this.hintInfo) {
      const { info, text } = this.data.list[i]
      this.selectedHintInfo = {
        info,
        text
      }
  ...
  scrollToActive: function () {
    ...
    var node2 = this.hints.childNodes[
      Math.min(this.data.list.length - 1, this.selectedHint + margin)
    ];
