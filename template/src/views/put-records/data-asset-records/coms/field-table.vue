<template>
  <el-row>
    <el-card class="box-card">
      <div class="field-table-header-box">
        <FilterBar
          v-model="filterValue"
          :fields="filterFields"
...
import TableRecordDialog from "../components/table-record-dialog.vue";
  ...
  methods: {
    ...
    toEdit(row) {
      ...
      if (Array.isArray(classifyList)) {
        const index = classifyList.findIndex(({ id }) => id === senTypeId);
    ...
    toRecord({ tableId, tableName, useInfo }) {
      this.selectedIds = [tableId];
