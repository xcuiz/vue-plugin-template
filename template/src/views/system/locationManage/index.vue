<template>
  <div class="location-manage-container">
    <div class="location-manage-box">
      <div class="header-box">
        <div class="header-title">物理位置管理</div>
        <PageHeader title="">
...
export default {
  ...
  data() {
    return {
      ...
      // 获取列表数据的url
      listUrl: "/gov/api/asset/location/page",
      // 模块名
  ...
  methods: {
    ...
    toDelete(ids) {
      this.$confirm("确定删除所选数据?", "提示", {
        ...
        try {
          await this.$request({
            url: "/gov/api/asset/location/delete",
            method: "POST",
            data: {
              idList: ids,
            },
    ...
    async toDownload() {
      ...
      try {
        const result = await this.$request(
          {
            url: "/gov/api/common/downloadTemp/location",
            method: "GET",
          },
          {
            responseType: "blob",
