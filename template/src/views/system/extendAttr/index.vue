<template>
  <div class="extend-attr-container">
    <div class="extend-attr-box">
      <div class="header-box">
        <div class="header-title">扩展属性管理</div>
        <PageHeader title="">
            ...
            >
              <template slot="left">
                <div class="filter-items">
                  ...
                  <div class="filter-items-btn">
                    <el-button size="mini" @click="toMultiDelete">
                      删除
                    </el-button>
                  </div>
                </div>
        ...
        <div class="name-list-content">
          ...
          <div v-loading="tableLoading" class="name-list-table">
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  {{ scope.row.propTypeValue || "-" }}
                </template>
              </el-table-column>
              <el-table-column label="应用数据类型" show-overflow-tooltip>
              </el-table-column>
              <el-table-column
                label="创建时间"
                show-overflow-tooltip
...
export default {
  ...
  data() {
    return {
      ...
      // 获取列表数据的url
      listUrl: "/gov/api/data/extend/page",
      // 模块名
  ...
  methods: {
    ...
    async toMultiStart() {
      if (this.selectedIds.length === 0) {
        this.$message.warning("请选择需要开启的数据");
      ...
      try {
        await this.$request({
          url: `/gov/api/data/extend/operate/0`,
          method: "POST",
          data: {
    ...
    async toMultiStop() {
      ...
      try {
        await this.$request({
          url: `/gov/api/data/extend/operate/2`,
          method: "POST",
          data: {
            idList: this.selectedIds,
          },
    ...
    toDelete(ids) {
      this.$confirm("确定删除所选数据?", "提示", {
        ...
        try {
          await this.$request({
            url: "/gov/api/data/extend/delete",
            method: "POST",
            data: {
              idList: ids,
            },
    ...
    async statusChange({ status, id }) {
      const s = status === 0 ? 2 : 0;
      const msgText = status === 0 ? "停用" : "开启";

      try {
        await this.$request({
          url: `/gov/api/data/extend/operate/${s}`,
          method: "POST",
          data: {
            idList: [id],
          },
