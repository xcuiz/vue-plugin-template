<template>
  <div class="application-manage-container">
    <div class="application-manage-box">
      <div class="header-box">
        <div class="header-title">应用系统管理</div>
        <PageHeader title="">
              ...
              >
                <template slot-scope="scope">
                  <el-button
                    ...
                    >{{ scope.row.businessSysName || "-" }}</el-button
                  >
                </template>
              </el-table-column>
              <el-table-column
                ...
                show-overflow-tooltip
                sortable="custom"
                prop="businessSysCode"
              >
                <template slot-scope="scope">
                  {{ scope.row.businessSysCode || "-" }}
                </template>
              </el-table-column>
              <el-table-column
                label="部门"
...
export default {
  ...
  data() {
    return {
      ...
      // 获取列表数据的url
      listUrl: "/gov/api/asset/businessSys/page",
      // 模块名
  ...
  methods: {
    ...
    toDelete(ids) {
      this.$confirm("确定删除所选数据?", "提示", {
        ...
        try {
          await this.$request({
            url: "/gov/api/asset/businessSys/delete",
            method: "POST",
            data: {
              idList: ids,
            },
    ...
    async toDownload() {
      ...
      try {
        const result = await this.$request(
          {
            url: "/gov/api/common/downloadTemp/businessSys",
            method: "GET",
          },
          {
            responseType: "blob",
