  ...
  <div class="event-classify-container">
    <div class="event-classify-box">
      <div class="event-classify-header">
        <div class="header-title">敏感元素管理</div>
      </div>
      ...
      <div class="event-classify-content">
        <div class="event-classify-table">
          ...
          >
            ...
            <el-table-column label="元素名称" show-overflow-tooltip prop="name">
              <template slot-scope="scope">
                <el-button
                  type="text"
                  size="mini"
  ...
  methods: {
    ...
    async getTableData() {
      ...
      const result = await this.$request({
        url: "/gov/api/strategy/sensitiveElement/page",
        method: "POST",
    ...
    toDelete() {
      ...
      this.$confirm("选中的数据将被删除,是否继续?", "提示", {
        ...
        .then(async () => {
          try {
            await this.$request({
              url: "/gov/api/strategy/sensitiveElement/delete",
              method: "POST",
              data: {
                ids: this.ids,
              },
    ...
    async statusChange(ids, status) {
      await this.$request({
        url: "/gov/api/strategy/sensitiveElement/changeStatus",
        method: "POST",
        data: {
          ids,
          status,
    ...
    handleStatus(status) {
      ...
      if (this.ids.length === 0) {
        this.$message({
          type: "warn",
          message: `请选择需要${statusText}的元素`,
        });

        return;
      }
      this.$confirm(`选中的元素将被${statusText},是否继续?`, "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
