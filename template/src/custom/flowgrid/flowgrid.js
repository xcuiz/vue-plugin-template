...
var dragdrop = {
	...
	position: function position() {
		...
		this.dragElement.style.cssText += 'left: ' + x + 'px; top: ' + y + 'px;';
		// 极值判断
	...
	resize: function resize() {
		...
		x = parseInt(this.dragElement.style.left.replace('px', '')),
		    y = parseInt(this.dragElement.style.top.replace('px', '')),
...
var fgContainer = {
  render: function render() {
    var _vm = this;var _h = _vm.$createElement;var _c = _vm._self._c || _h;return _c('div', { staticClass: "fg-container" }, [_c('div', { staticClass: "fg-layout", class: { 'fg-no-draggable': !_vm.opt.draggable, 'fg-no-resizable': !_vm.opt.resizable }, style: _vm.itemStyle, attrs: { "index": _vm.index } }, [_vm._t("default"), _vm._v(" "), _c('div', { directives: [{ name: "show", rawName: "v-show", value: _vm.isDrag, expression: "isDrag" }], staticClass: "fg-item-dragdrop" })], 2)]);
  },
  staticRenderFns: [],
  name: 'fg-container',
  props: ['setting', 'nodes'],
  ...
  methods: {
    ...
    init: function init() {
      this.handleEvent.init(true, this.opt);
      // 缓存对象
      cache.init();
  ...
  computed: {
    itemStyle: function itemStyle() {
      ...
      return {
        ...
        top: node.y * opt.cellH + 2 * opt.padding.top + 'px',
        left: node.x * opt.cellW + 2 * opt.padding.left + 'px',
        width: this.width + 'px',
