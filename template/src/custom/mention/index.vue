  ...
  methods: {
    ...
    checkKey() {
      ...
      if (index >= 0) {
        ...
        const isInputInsert =
          searchText &&
          (this.insert.indexOf(searchText.charAt(searchText.length - 1)) !==
            -1 ||
            (searchText.length > 2 &&
              this.insert.indexOf(searchText.charAt(searchText.length - 2)) !==
                -1));
        if (searchText != null && !isInputInsert) {
    ...
    applyMention(itemIndex) {
      ...
      const value =
        ...
        this.insert;
      this.setValue(
        this.replaceText(this.getValue(), this.searchText, value, this.keyIndex)
      );
